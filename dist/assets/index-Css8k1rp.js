(function(){const s=document.createElement("link").relList;if(s&&s.supports&&s.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))n(i);new MutationObserver(i=>{for(const a of i)if(a.type==="childList")for(const l of a.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&n(l)}).observe(document,{childList:!0,subtree:!0});function t(i){const a={};return i.integrity&&(a.integrity=i.integrity),i.referrerPolicy&&(a.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?a.credentials="include":i.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function n(i){if(i.ep)return;i.ep=!0;const a=t(i);fetch(i.href,a)}})();const ua="modulepreload",va=function(e){return"/"+e},rt={},Q=function(s,t,n){let i=Promise.resolve();if(t&&t.length>0){document.getElementsByTagName("link");const l=document.querySelector("meta[property=csp-nonce]"),r=(l==null?void 0:l.nonce)||(l==null?void 0:l.getAttribute("nonce"));i=Promise.allSettled(t.map(c=>{if(c=va(c),c in rt)return;rt[c]=!0;const v=c.endsWith(".css"),p=v?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${c}"]${p}`))return;const g=document.createElement("link");if(g.rel=v?"stylesheet":ua,v||(g.as="script"),g.crossOrigin="",g.href=c,r&&g.setAttribute("nonce",r),document.head.appendChild(g),v)return new Promise((f,u)=>{g.addEventListener("load",f),g.addEventListener("error",()=>u(new Error(`Unable to preload CSS for ${c}`)))})}))}function a(l){const r=new Event("vite:preloadError",{cancelable:!0});if(r.payload=l,window.dispatchEvent(r),!r.defaultPrevented)throw l}return i.then(l=>{for(const r of l||[])r.status==="rejected"&&a(r.reason);return s().catch(a)})},me={location:null,powerSource:null,tariffBand:null,gridSpend:1e5,fuelSpend:5e4,generatorSize:null,houseType:null,rooms:0,appliances:[],solarAppliances:null,customSchedule:null,goal:null,backupHours:4,budget:15e5,results:null,_data:{}},ma=new Set;function R(){return{...me}}function D(e){Object.assign(me,e),ma.forEach(s=>s({...me}))}function z(e){return me._data[e]||null}function ee(e,s){me._data[e]=s}const ga=['<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><circle cx="12" cy="12" r="4"/><path d="M12 2v2M12 20v2M4.93 4.93l1.41 1.41M17.66 17.66l1.41 1.41M2 12h2M20 12h2M4.93 19.07l1.41-1.41M17.66 6.34l1.41-1.41"/></svg>','<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M13 2L4.09 12.33A1 1 0 005 14h7l-1 8 8.91-10.33A1 1 0 0019 10h-7l1-8z"/></svg>','<svg width="19" height="19" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"/><circle cx="11" cy="11" r="4"/><line x1="21" y1="3" x2="14.35" y2="9.65"/><polyline points="17,3 21,3 21,7"/></svg>'],lt=[[1],[2],[3]],ya=["step1","step2","step3"];function Ke(e){const s=lt.findIndex(t=>t.includes(e));return`
    <div class="progress-bar">
      ${lt.map((t,n)=>{const i=n<s,a=n===s,l=n<=s,r=i?"completed":a?"active":"",c=l?`onclick="window._navigate('${ya[n]}')" style="cursor:pointer"`:'style="cursor:default"';return`
          ${n>0?`<div class="progress-bar__line ${i?"completed":""}"></div>`:""}
          <div class="progress-bar__step">
            <div class="progress-bar__dot ${r}" ${c}>${ga[n]}</div>
          </div>
        `}).join("")}
    </div>
  `}function Tt(e,s){var r,c,v;const t=R(),n=z("pv_yield")||[];e.innerHTML=`
        ...
        <div class="card" style="max-width:480px">
          ...
          <div class="select-wrap">
            <select class="sunova-select" id="location-select">
              ...
              ${n.map(p=>{var g;return`
                <option value="${p.state}" ${((g=t.location)==null?void 0:g.state)===p.state?"selected":""}>
                  ${p.state} (${p.zone})
                </option>
              `}).join("")}
            </select>
  ...
  `;const i=document.getElementById("location-select"),a=document.getElementById("continue-btn"),l=document.getElementById("location-info");i.addEventListener("change",()=>{const p=n.find(g=>g.state===i.value);p?(D({location:p}),document.getElementById("loc-zone").textContent=p.zone,document.getElementById("loc-psh").textContent=`${p.daily_yield_kwh_per_kwp} hrs/day`,document.getElementById("loc-yield").textContent=`${p.annual_yield_kwh_per_kwp} kWh/kWp`,l.style.display="block",a.disabled=!1):(D({location:null}),l.style.display="none",a.disabled=!0)}),a.addEventListener("click",()=>{R().location&&s("step2")})}function we(e){return"₦"+Number(e).toLocaleString("en-NG")}function ct(e){return parseInt(String(e).replace(/[₦,\s]/g,""),10)||0}function Bt(e){return(14-e/100*28).toFixed(1)}function dt({id:e,value:s,min:t,max:n,step:i=1e3,ticks:a,label:l="per month",formatFn:r=we}){const c=(s-t)/(n-t)*100,v=Bt(c),p=a?a.map((g,f)=>{const u=(g-t)/(n-t)*100,o=s>=(f===0?t:a[f-1])&&s<=g,d=f===0?"translateX(0)":f===a.length-1?"translateX(-100%)":"translateX(-50%)";return`<span class="slider-tick ${o?"active":""}" data-val="${g}" style="left:${u}%;transform:${d}">${r(g)}</span>`}).join(""):"";return`
    <div class="slider-wrapper">
      ...
      <div class="slider-track-wrap">
        <div class="slider-tooltip" id="${e}-tooltip" style="left:calc(${c}% + ${v}px)">
          <span class="slider-tooltip__val">${r(s)}</span>
  ...
  `}function pt(e,s=we,t){const n=document.getElementById(e),i=document.getElementById(`${e}-val`),a=document.getElementById(`${e}-tooltip`);if(!n||!i)return;const l=Number(n.min),r=Number(n.max),c=Number(n.step)||1;function v(){const p=Number(n.value),g=(p-l)/(r-l)*100,f=Bt(g);if(n.style.background=`linear-gradient(to right, var(--color-primary) ${g}%, var(--color-border) ${g}%)`,i.value=s(p),a){a.style.left=`calc(${g}% + ${f}px)`;const o=a.querySelector(".slider-tooltip__val");o&&(o.textContent=s(p))}t&&t(p);const u=n.closest(".slider-track-wrap").querySelectorAll(".slider-tick");if(u.length>0){const o=Array.from(u).map(d=>Number(d.dataset.val));u.forEach((d,y)=>{const E=y===0?l:o[y-1];d.classList.toggle("active",p>=E&&p<=o[y])})}}if(n.addEventListener("input",v),n.addEventListener("change",v),i.addEventListener("focus",()=>{i.value=ct(i.value)||"",i.select()}),i.addEventListener("blur",()=>{const p=ct(i.value),g=Math.max(l,Math.min(r,p||l)),f=Math.round((g-l)/c)*c+l;n.value=f,v()}),i.addEventListener("keydown",p=>{p.key==="Enter"&&i.blur()}),a){const p=()=>a.classList.add("visible"),g=()=>a.classList.remove("visible");n.addEventListener("mousedown",p),n.addEventListener("touchstart",p,{passive:!0}),document.addEventListener("mouseup",g),document.addEventListener("touchend",g)}}function De({cards:e,selected:s,name:t}){return`
  ...
  `}function je(e,s){const t=document.querySelector(`[data-radio-group="${e}"]`);t&&t.querySelectorAll(".radio-card").forEach(n=>{n.addEventListener("click",()=>{t.querySelectorAll(".radio-card").forEach(i=>i.classList.remove("selected")),n.classList.add("selected"),s&&s(n.dataset.value)})})}const ha='<img src="/icons/grid_only.png" width="72" height="72" style="object-fit:contain">',fa='<img src="/icons/generator_only.png" width="72" height="72" style="object-fit:contain">',ba='<img src="/icons/grid_and_generator.png" width="72" height="72" style="object-fit:contain">',ka='<img src="/icons/small_generator_i_better_pass_my_neighbour.png" width="72" height="72" style="object-fit:contain">',xa='<img src="/icons/medium_generator.png" width="72" height="72" style="object-fit:contain">',_a='<img src="/icons/silent_diesel_generator.png" width="72" height="72" style="object-fit:contain">',wa=[{id:"grid_only",emoji:ha,name:"Grid Only",desc:"I rely solely on NEPA/DisCo grid supply"},{id:"both",emoji:ba,name:"Grid + Generator",desc:"I use both grid and a backup generator"},{id:"generator_only",emoji:fa,name:"Generator Only",desc:"No grid, I run a generator for power"}],Sa=[{id:"small",emoji:ka,label:"I better pass my neighbour",name:"Small (1–2 KVA)"},{id:"medium",emoji:xa,label:"Gasoline generator",name:"Medium (3–5 KVA)"},{id:"large",emoji:_a,label:"Silent diesel generator",name:"Large (6–10 KVA)"}];function Rt(e,s){function t(){var v,p;const i=R(),a=z("tariff_bands")||[],l=i.powerSource==="grid_only"||i.powerSource==="both",r=i.powerSource==="generator_only"||i.powerSource==="both",c=i.powerSource&&(!l||!!i.tariffBand)&&(!r||!!i.generatorSize);e.innerHTML=`
      <div class="wizard-step">
        <div class="wizard-header">
        ...
        <div class="step-body">
          ...
          ${De({cards:wa,selected:i.powerSource,name:"power-source"})}
              ...
              <div class="card" style="margin-top:10px;margin-bottom:16px;background:var(--color-primary-bg);border-color:var(--color-primary-light);padding:10px 14px">
                <div style="display:flex;align-items:center;gap:14px">
                  ...
                  <div style="font-size:12px;color:var(--color-text-secondary)">
                    ...
                    &nbsp;·&nbsp; ₦${((p=a.find(g=>g.band===i.tariffBand))==null?void 0:p.tariff_naira_per_kwh)||""}/kWh
          ...
          ${r?`
            ...
            ${De({cards:Sa,selected:i.generatorSize,name:"gen-size"})}
    ...
    `,n()}function n(){document.getElementById("back-btn").addEventListener("click",()=>s("step1")),document.getElementById("continue-btn").addEventListener("click",()=>s("step3")),je("power-source",i=>{D({powerSource:i}),t()}),document.querySelectorAll(".tariff-pill").forEach(i=>{i.addEventListener("click",()=>{D({tariffBand:i.dataset.band}),t()})}),je("gen-size",i=>{D({generatorSize:i}),t()}),document.getElementById("grid-spend-slider")&&pt("grid-spend-slider",we,i=>D({gridSpend:i})),document.getElementById("fuel-spend-slider")&&pt("fuel-spend-slider",we,i=>D({fuelSpend:i}))}t()}const ut=["Analysing your energy profile","Sizing your solar PV system","Calculating your savings with solar","Finalising your results"],Ea=[{activeAt:0,doneAt:3e3},{activeAt:3e3,doneAt:8e3},{activeAt:8e3,doneAt:15e3},{activeAt:15e3,doneAt:18e3}],Aa=18e3,$a=`
#preloader-overlay {
  position: fixed;
  inset: 0;
  background: #ffffff;
...
`;function La(){if(document.getElementById("pl-styles"))return;const e=document.createElement("style");e.id="pl-styles",e.textContent=$a,document.head.appendChild(e)}function Ca(e){La();const s=document.createElement("div");s.id="preloader-overlay",s.innerHTML=`
  ...
  `,document.body.appendChild(s),Ea.forEach(({activeAt:t,doneAt:n},i)=>{setTimeout(()=>{var a;(a=document.getElementById(`pl-step-${i}`))==null||a.classList.add("active")},t),setTimeout(()=>{const a=document.getElementById(`pl-step-${i}`);a&&(a.classList.remove("active"),a.classList.add("done"))},n)}),setTimeout(()=>{const t=document.getElementById("preloader-overlay");t&&(e(),t.style.opacity="0",setTimeout(()=>{t.remove()},400))},Aa)}function Wt(e,s,t,n,i){const{gridSpend:a=0,fuelSpend:l=0,tariffBand:r,generatorSize:c,powerSource:v,location:p,appliances:g=[],customSchedule:f}=e;let u=0;if(a>0&&r&&v!=="generator_only"){const h=(t||[]).find(F=>F.band===r),$=(h==null?void 0:h.tariff_naira_per_kwh)||0;$>0&&(u=a/$/30)}let o=0;if(l>0&&c&&v!=="grid_only"){const{fuelTypeStr:h,kwhPerLitre:$}=Ma(c,i),F=(p==null?void 0:p.state)||"",T=Fa(h,F,n);T>0&&$>0&&(o=l/T*$/30)}let d=u+o;d<1&&(d=Ta(g,s));const y={};(s||[]).forEach(h=>{y[h.name]=h});const E={};(g||[]).forEach(h=>{E[h.name]=h.qty||1});const A=new Array(24).fill(0);f&&f.length>0&&f.forEach(h=>{const $=y[h.name];if(!$)return;const F=E[h.name]||1,T=$.rated_watts*F/1e3;(h.segments||[]).forEach(x=>{const B=x.start??x.start_hour??0,W=x.end??x.end_hour??0;for(let I=0;I<24;I++)Math.min(W,I+1)-Math.max(B,I)>0&&(A[I]+=T)})});const w=A.reduce((h,$)=>h+$,0),M=u+o>0,_=d;if(w>0){if(_>0){const h=w/_;u=parseFloat((u*h).toFixed(2)),o=parseFloat((o*h).toFixed(2))}d=w}const b=w>0?A.map(h=>parseFloat(h.toFixed(3))):new Array(24).fill(parseFloat((d/24).toFixed(3))),m=Math.max(0,d-w);let k,S;if(w===0)k=30,S="Low";else if(!M)k=55,S="Medium";else{const h=Math.abs(w-_)/Math.max(w,_);h<=.25?(k=Math.round(99-h/.25*14),S="High"):h<=.6?(k=Math.round(84-(h-.25)/.35*29),S="Medium"):(k=Math.max(30,Math.round(54-(h-.6)/.4*24)),S="Low")}const C=b.reduce((h,$,F)=>$>b[h]?F:h,0),L=parseFloat(b[C].toFixed(2));return{totalDailyKWh:parseFloat(d.toFixed(2)),dailyGridKWh:parseFloat(u.toFixed(2)),dailyGenKWh:parseFloat(o.toFixed(2)),ganttTotalKWh:parseFloat(w.toFixed(2)),unaccountedKWh:parseFloat(m.toFixed(2)),hourlyProfile:b,ganttHourly:A.map(h=>parseFloat(h.toFixed(3))),peakHour:C,peakKW:L,confidenceScore:k,confidenceLabel:S,confidenceReason:w===0?"no_appliances":M?S==="High"?"ok":"variance":"no_spending",confidenceDirection:!M||w===0?null:w>_?"appliances_higher":w<_?"spending_higher":"matched",monthlyKWh:parseFloat((d*30).toFixed(1))}}function Ma(e,s){const t={small:{fuelTypeStr:"PMS",kwhPerLitre:2.27},medium:{fuelTypeStr:"PMS",kwhPerLitre:3.38},large:{fuelTypeStr:"AGO",kwhPerLitre:3.71}};if(!s||s.length===0)return t[e]||t.medium;const n={small:{types:["Small Portable"],preferFuel:"PMS"},medium:{types:["Mid-size"],preferFuel:"PMS"},large:{types:["Mid-size","Large Home"],preferFuel:"AGO"}},i=n[e]||n.medium;let a=s.filter(r=>i.types.includes(r.type)&&r.fuel_type.includes(i.preferFuel));if(a.length===0&&(a=s.filter(r=>i.types.includes(r.type))),a.length===0)return t[e]||t.medium;const l=a.reduce((r,c)=>r+c.kwh_per_litre,0)/a.length;return{fuelTypeStr:i.preferFuel,kwhPerLitre:l}}function Fa(e,s,t){const n={AGO:1800,PMS:1300};if(!t||t.length===0)return n[e]||1100;const i=e==="AGO"?"AGO":"PMS",a=t.filter(r=>r.fuel_type.includes(i));if(a.length===0)return n[e]||1100;const l=a.find(r=>r.state.toLowerCase()===s.toLowerCase());return l?l.price_per_litre_naira:Math.round(a.reduce((r,c)=>r+c.price_per_litre_naira,0)/a.length)}function Ta(e,s){if(!e||e.length===0)return 5;const t={};(s||[]).forEach(i=>{t[i.name]=i});let n=0;return e.forEach(i=>{const a=t[i.name];a&&(n+=a.daily_wh*(i.qty||1)/1e3)}),Math.max(2,n)}const We=585,vt=.75,Ba=.8,Ra=1.25,Wa=6.5,Ia=.9025,Pa=6,qa=18,Ie=[3,5,7.5,10,12.5,15,20,25,30],Oa=25e4,Ga=6e4,Da=15e4;function It(e,s,t=0){const n=e.totalDailyKWh,i=Math.max(e.peakKW,t),a=(s==null?void 0:s.daily_yield_kwh_per_kwp)||4.5,l=(s==null?void 0:s.annual_yield_kwh_per_kwp)||1642,r=e.hourlyProfile||[];let c=0,v=0;if(r.length===24)for(let m=0;m<24;m++)m>=Pa&&m<qa?c+=r[m]:v+=r[m];else c=n*.5,v=n*.5;const p=c+v/Ia,g=p/(a*vt),f=p*365/l,u=Math.max(g,f),o=Math.ceil(u*1e3/We),d=parseFloat((o*We/1e3).toFixed(2)),y=i*Ra/Ba,E=Ie.find(m=>m>=y)??Ie[Ie.length-1],A=parseFloat((d*Wa).toFixed(1)),w=Math.round(d*l),_=["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"].map(m=>{var k;return{month:m,kwh:Math.round(d*(((k=s==null?void 0:s.monthly)==null?void 0:k[m.toLowerCase()])||a)*30*vt)}}),b=Math.round(d*Oa+E*Ga+Da);return{panel_kwp:d,panel_count:o,inverter_kva:E,installation_m2:A,annual_gen_kwh:w,monthly_gen:_,estimated_cost:b,psh:a,pvKWp_required:parseFloat(u.toFixed(2)),method1_kWp:parseFloat(g.toFixed(2)),method2_kWp:parseFloat(f.toFixed(2)),panelWattage:We}}const Pe=.8,mt=.95,gt=18,yt=23,ja=[18,19,20,21,22,23,0,1,2,3,4,5],Na=1.5,qe=[5,10,15,20,25,30,40,50,60,80,100],za={reduce_bill:4,backup:8,offgrid:16},Ha=.25,Ka=.6;function Pt(e,s,t){const n=e.peakKW||1,i=e.totalDailyKWh||1,a=e.hourlyProfile||[],l=s==="backup"&&t>0?t:za[s]??4,r=a.slice(gt,yt+1),c=r.reduce((w,M)=>w+M,0);let v=r.length>0?c/r.length:0;v===0&&(v=n*.6);let p;s==="offgrid"?p=(a.length===24?ja.reduce((M,_)=>M+(a[_]||0),0):i*.5)*Na:p=v*l;const f=p/Pe/mt,u=qe.find(w=>w>=f)??qe[qe.length-1],o=parseFloat((u*Pe*mt).toFixed(2)),d=n>0?Math.min(24,Math.round(o/(n*Ha))):24,y=n>0?Math.min(24,Math.round(o/(n*Ka))):24,E=n>0?Math.min(24,Math.round(o/n)):24,A=Math.ceil(u/5);return{battery_kwh:u,battery_units_48v:A,storage_capacity:u,storage_output:parseFloat(v.toFixed(2)),backup_hours_essentials:d,backup_hours_appliances:y,backup_hours_whole_home:E,backupHours:l,avgBackupLoad_kW:parseFloat(v.toFixed(2)),energyNeeded_kWh:parseFloat(p.toFixed(2)),batteryKWh_gross:parseFloat(f.toFixed(2)),batteryKWh_recommended:u,dod:Pe,backupWindowStart:gt,backupWindowEnd:yt}}function qt({hourlyProfile:e,pvKWp:s,batteryKWh:t,dailyYield:n,dod:i=.8,batteryEfficiency:a=.95,energyMix:l="grid_and_generator",sunriseHour:r=6,sunsetHour:c=18}){const v=Array.from({length:24},(L,h)=>{const $=(h-r)/(c-r)*Math.PI;return $>0&&$<Math.PI?Math.sin($):0}),p=v.reduce((L,h)=>L+h,0),g=v.map(L=>p>0?s*n*L/p:0),f=t,u=t*(1-i);let o=f;for(let L=c;L<24;L++){const h=e[L]||0,$=Math.max(0,o-u),F=Math.min(h,$*a);o-=F/a}let d=Math.max(u,o);const y=Array.from({length:24},(L,h)=>{const $=e[h]||0,F=g[h],T=Math.min(F,$),x=Math.max(0,F-$),B=Math.max(0,$-F),W=Math.min(x,(f-d)/a);d+=W*a;const I=W,j=Math.max(0,d-u),H=Math.min(B,j*a);d-=H/a;const J=H,ne=B-H;return{hour:h,demand:parseFloat($.toFixed(3)),solar_to_load:parseFloat(T.toFixed(3)),battery_to_load:parseFloat(J.toFixed(3)),grid_to_load:parseFloat(ne.toFixed(3)),solar_to_charge:parseFloat(I.toFixed(3)),soc_end:parseFloat(d.toFixed(3)),soc_pct:parseFloat((d/t*100).toFixed(1))}}),E=y.reduce((L,h)=>L+h.demand,0),A=y.reduce((L,h)=>L+h.solar_to_load,0),w=y.reduce((L,h)=>L+h.battery_to_load,0),M=y.reduce((L,h)=>L+h.grid_to_load,0),_=y.reduce((L,h)=>L+h.solar_to_charge,0),b=1,m=E>0?M/E:0,k=parseFloat(_.toFixed(2)),S=parseFloat(M.toFixed(2)),C=l==="grid_only"?"Grid":l==="generator_only"?"Generator":"Grid+Gen";return{hours:y,totalDemand:parseFloat(E.toFixed(2)),totalSolarToLoad:parseFloat(A.toFixed(2)),totalBatteryToLoad:parseFloat(w.toFixed(2)),totalGridToLoad:parseFloat(M.toFixed(2)),totalSolarCharge:parseFloat(_.toFixed(2)),gridReliance_before:b,gridReliance_after:parseFloat(m.toFixed(2)),dailySurplusKWh:k,avgDailyGridKWh:S,gridLabel:C}}const Ya=26e4,ht=28e4,Va=2e5,Ua=.15,Ja=10,te=25,ft=.005,Xa=.07,bt=.43,Za=.65,Qa=.7,kt={small:{types:["Small Portable"],preferFuel:"PMS",defaultKwh:2.27},medium:{types:["Mid-size"],preferFuel:"PMS",defaultKwh:3.38},large:{types:["Mid-size","Large Home"],preferFuel:"AGO",defaultKwh:3.71}};function es(e,s){const t=kt[e]||kt.medium,n=s||[];let i=n.filter(l=>t.types.includes(l.type)&&l.fuel_type.includes(t.preferFuel));i.length||(i=n.filter(l=>t.types.includes(l.type)));const a=i.length?i.reduce((l,r)=>l+r.kwh_per_litre,0)/i.length:t.defaultKwh;return{fuelTypeStr:t.preferFuel,kwhPerLitre:a}}function ts(e,s,t){const n={AGO:1800,PMS:1300};if(!(t!=null&&t.length))return n[e]||1100;const i=e==="AGO"?"AGO":"PMS",a=t.filter(r=>r.fuel_type.includes(i));if(!a.length)return n[e]||1100;const l=a.find(r=>{var c;return((c=r.state)==null?void 0:c.toLowerCase())===(s||"").toLowerCase()});return l?l.price_per_litre_naira:Math.round(a.reduce((r,c)=>r+c.price_per_litre_naira,0)/a.length)}function as({load:e,solar:s,battery:t,dispatch:n,tariffData:i,fuelPrices:a,genData:l,state:r}){var tt,at,st;const c=e.totalDailyKWh,v=e.monthlyKWh??e.totalDailyKWh*30,p=e.dailyGridKWh||0,g=e.dailyGenKWh||0,f=s.panel_kwp,u=s.inverter_kva,o=((tt=r.location)==null?void 0:tt.annual_yield_kwh_per_kwp)||(((at=r.location)==null?void 0:at.daily_yield_kwh_per_kwp)||4.5)*365,d=t.battery_kwh,y=r.powerSource||"grid_only",E=r.goal||"reduce_bill",A=r.budget||0,w=i==null?void 0:i.find(Y=>Y.band===r.tariffBand),M=(w==null?void 0:w.tariff_naira_per_kwh)||194,{fuelTypeStr:_,kwhPerLitre:b}=es(r.generatorSize,l),m=((st=r.location)==null?void 0:st.state)||"",k=ts(_,m,a),S=y!=="grid_only"?Math.round(k/b):0,C=Math.round(f*Ya+d*ht+u*Va),L=Math.round(C*Ua),h=C+L,$=Math.round(d*ht),F=A>0?h<=A:!0,T=Math.max(0,A-h),x=Math.max(0,h-A),B=p+g;let W,I;B>0?(W=p/B,I=g/B):y==="grid_only"?(W=1,I=0):y==="generator_only"?(W=0,I=1):(W=.5,I=.5);let j;y==="grid_only"?j=M:y==="generator_only"?j=S:j=Math.round(W*M+I*S);const H=Math.round(j*v),J=f*o*((1-Math.pow(1-ft,te))/ft),ie=J>0?Math.round((h+$)/J):45,{totalDemand:ne,totalSolarToLoad:sa,totalBatteryToLoad:ia,totalGridToLoad:na}=n,de=ne>0?(sa+ia)/ne:0,Ue=ne>0?na/ne:1-de,Fe=Ue*W,ye=Ue*I;let oe;E==="offgrid"?oe=ie:y==="grid_only"?oe=Math.round(de*ie+Fe*M):y==="generator_only"?oe=Math.round(de*ie+ye*S):oe=Math.round(de*ie+Fe*M+ye*S);const Je=Math.round(oe*v),Xe=H-Je,pe=Xe*12,oa=pe>0?parseFloat((h/pe).toFixed(1)):99,Te=pe*te,ra=Math.round((Te-h-$)/h*100),Ze=h+$,la=Te>0&&Ze>0?parseFloat(((Math.pow(Te/Ze,1/te)-1)*100).toFixed(1)):0;let Be=0,Qe=0;if(y!=="grid_only"){const it=Math.max(0,g-ye*c)*365;Be=b>0?Math.round(it/b):0,Qe=Math.round(Be*k)}let he;const et=_==="AGO"?Qa:Za;y==="grid_only"?he=bt:y==="generator_only"?he=et:he=W*bt+I*et;const ca=f*o,da=parseFloat((ca*he/1e3).toFixed(1));let re=-h,le=-1,fe=null;const Re=[{year:0,cumulative:re}];for(let Y=1;Y<=te;Y++){const nt=pe*Math.pow(1+Xa,Y)-(Y===Ja?$:0),ot=re;re+=nt,le===-1&&ot<0&&re>=0&&(le=Y,fe=parseFloat((Y-1+Math.abs(ot)/nt).toFixed(1))),Re.push({year:Y,cumulative:Math.round(re)})}le===-1&&(re>=0?(le=te,fe=te):(le=99,fe=99));const pa=y==="grid_only"?"Grid":y==="generator_only"?"Generator":"Grid + Gen";return{total_system_cost:h,equipment_cost:C,bos_cost:L,battery_replacement_cost:$,isWithinBudget:F,budgetSurplus:T,budgetShortfall:x,current_blended_cost:j,post_solar_blended_cost:oe,LCOE:ie,gen_cost_per_kwh:S,current_monthly_cost:H,post_solar_monthly_cost:Je,monthly_savings:Xe,annual_savings:pe,simple_payback_years:oa,ROI:ra,annualised_ROI:la,lifetime_savings:Re[te].cumulative,litres_saved_per_year:Be,fuel_naira_saved_annual:Qe,co2_avoided_tonnes:da,solar_fraction:parseFloat(de.toFixed(3)),grid_fraction:parseFloat(Fe.toFixed(3)),gen_fraction:parseFloat(ye.toFixed(3)),cashflow:Re,payback_year_index:le,payback_exact:fe,current_label:pa,solar_label:"With Solar"}}const ss=[{name:"Split AC – 1HP",qty:1},{name:"Ceiling Fan",qty:3},{name:"LED Bulb (9W)",qty:18},{name:"LED Bulb (15W)",qty:9},{name:"Security Light (floodlight)",qty:5},{name:"Refrigerator (200L)",qty:1},{name:'LED TV – 43"',qty:2},{name:"DSTV Decoder",qty:1},{name:"Wi-Fi Router",qty:1},{name:"Phone Charger",qty:2},{name:"CCTV System (4 cameras)",qty:1}];function is(e){const s=Object.fromEntries(e.map(t=>[t.name,t]));return ss.reduce((t,n)=>{const i=s[n.name];return t+(i?i.rated_watts*n.qty/1e3:0)},0)}function $e(){var o;const e=R(),s=z("appliances")||[],t=z("tariff_bands")||[],n=z("fuel_prices")||[],i=z("generator_efficiency")||[],a=new Set((e.appliances||[]).map(d=>d.name)),l=e.solarAppliances?new Set(e.solarAppliances):a,r={...e,appliances:(e.appliances||[]).filter(d=>l.has(d.name)),customSchedule:e.customSchedule?e.customSchedule.filter(d=>l.has(d.name)):null},c=Wt(r,s,t,n,i),v=is(s),p=It(c,e.location,v),g=Pt(c,e.goal,e.backupHours),f=qt({hourlyProfile:c.hourlyProfile,pvKWp:p.panel_kwp,batteryKWh:g.battery_kwh,dailyYield:((o=e.location)==null?void 0:o.daily_yield_kwh_per_kwp)||4.5,energyMix:e.powerSource}),u=as({load:c,solar:p,battery:g,dispatch:f,tariffData:t,fuelPrices:n,genData:i,state:e});D({results:{load:c,solar:p,battery:g,dispatch:f,savings:u}})}const ns=[{id:"reduce_bill",emoji:'<img src="/icons/reduce_my_bill.png" width="72" height="72" style="object-fit:contain">',name:"Reduce Bill",desc:"Reduce my monthly bill effectively"},{id:"backup",emoji:'<img src="/icons/backup_power.png" width="72" height="72" style="object-fit:contain">',name:"Backup Power",desc:"Have some backup power"},{id:"offgrid",emoji:'<img src="/icons/off_grid.png" width="72" height="72" style="object-fit:contain">',name:"Off-Grid",desc:"Go completely off-grid"}];function Ot(e,s){const t=R();e.innerHTML=`
    <div class="wizard-step">
      <div class="wizard-header">
        <button class="back-btn" id="back-btn">← Back</button>
        ${Ke(3)}
      ...
      <div class="step-body">
        ...
        ${De({cards:ns,selected:t.goal,name:"solar-goal"})}
      </div>
  ...
  `,document.getElementById("back-btn").addEventListener("click",()=>s("step2"));function n(){var l;(l=document.querySelector(".backup-hours-inject"))==null||l.remove();const i=document.querySelector('[data-radio-group="solar-goal"] [data-value="backup"]');if(!i)return;const a=document.createElement("div");a.className="backup-hours-inject",a.style.cssText="margin-top:12px;padding-top:12px;border-top:1.5px solid rgba(0,0,0,0.12);display:flex;justify-content:center",a.innerHTML=`
      ...
      </div>`,i.appendChild(a),document.getElementById("backup-dec").addEventListener("click",r=>{r.stopPropagation(),D({backupHours:Math.max(1,R().backupHours-1)}),document.getElementById("backup-val").textContent=R().backupHours}),document.getElementById("backup-inc").addEventListener("click",r=>{r.stopPropagation(),D({backupHours:R().backupHours+1}),document.getElementById("backup-val").textContent=R().backupHours})}je("solar-goal",i=>{var a;D({goal:i}),document.getElementById("generate-btn").disabled=!1,i==="backup"?n():(a=document.querySelector(".backup-hours-inject"))==null||a.remove()}),R().goal==="backup"&&n(),document.getElementById("generate-btn").addEventListener("click",()=>{$e(),Ca(()=>s("costSavings"))})}const xt=e=>"₦"+Number(e).toLocaleString("en-NG"),os=["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"],rs={north:[.6,.8,1.3,1.45,1.3,.85,.7,.72,.9,1.1,.75,.6],middle:[.75,.9,1.25,1.35,1.15,.82,.68,.7,.85,1.05,.85,.72],south:[1.1,1.2,1.3,1.2,1,.7,.65,.65,.8,1,1.1,1.05]};function Gt(e){return e?/North West|North East/i.test(e)?"north":/North Central/i.test(e)?"middle":"south":"south"}function ls(e,s,t,n){const i={};(t||[]).forEach(o=>{i[o.name]=o});let a=0,l=0;(s||[]).forEach(o=>{const d=i[o.name];if(!d)return;const y=d.rated_watts*(o.qty||1);l+=y,d.category==="Cooling"&&(a+=y)});const r=l>0?a/l:.35,c=rs[Gt(n)],v=c.reduce((o,d)=>o+d,0)/12,p=c.map(o=>o/v),g=e.totalDailyKWh,f=g*r,u=g-f;return p.map(o=>parseFloat(((u+f*o)*30).toFixed(1)))}function Dt(e,s){const t=R(),n=z("appliances")||[],{results:i,location:a,powerSource:l,tariffBand:r,gridSpend:c,fuelSpend:v,appliances:p}=t;if(!i){s("step1");return}const{load:g,solar:f}=i,u=p&&p.length>0,o={grid_only:"Grid Only",generator_only:"Generator Only",both:"Grid & Generator"}[l]||"Grid & Generator",d={};n.forEach(S=>{d[S.name]=S});let y=0,E=0;(p||[]).forEach(S=>{const C=d[S.name];if(!C)return;const L=C.rated_watts*(S.qty||1);E+=L,C.category==="Cooling"&&(y+=L)});const A=E>0?Math.round(y/E*100):35,w=(a==null?void 0:a.zone)||"",M=Gt(w),_={north:"Northern Nigeria: peak cooling March to May, cool harmattan Nov to Feb",middle:"Middle Belt: peak cooling March to May, mild rainy dip Jun to Sep",south:"Southern Nigeria: peak cooling Feb to Apr, rainy season dip Jun to Sep"}[M],b=u?`
    <div class="card">
  ...
  `,window._navigate=s,!u)return;const m=ls(g,p,n,w),k=parseFloat((m.reduce((S,C)=>S+C,0)/12).toFixed(1));_t(g),Oe(`Hourly kWh from your appliance schedule, scaled to match spending data. Confidence: ${g.confidenceLabel} (${g.confidenceScore}%).`),document.getElementById("chart-view-sel").addEventListener("change",function(){this.value==="hourly"?(_t(g),Oe(`Hourly kWh from your appliance schedule, scaled to match spending data. Confidence: ${g.confidenceLabel} (${g.confidenceScore}%).`)):(cs(m,k),Oe(`Seasonal estimate: ${A}% of your load is cooling. ${_}.`))})}function Oe(e){const s=document.getElementById("chart-caption");s&&(s.textContent=e)}function _t(e){var a;const s=(a=document.getElementById("load-chart"))==null?void 0:a.getContext("2d");if(!s)return;window._loadChart&&window._loadChart.destroy();const t=Array.from({length:24},(l,r)=>r===0?"12am":r===12?"12pm":r<12?`${r}am`:`${r-12}pm`),n=Math.max(...e.hourlyProfile),i=e.hourlyProfile.map(l=>{const r=l/n;return r>.75?"#EF4444":r>.45?"#F5A623":"#FCD34D"});window._loadChart=new Chart(s,{type:"bar",data:{labels:t,datasets:[{data:e.hourlyProfile,backgroundColor:i,borderRadius:8,borderSkipped:!1,barPercentage:.55,categoryPercentage:.8}]},options:{responsive:!0,plugins:{legend:{display:!1},tooltip:{callbacks:{label:l=>`${l.raw} kW`}}},scales:{x:{grid:{display:!1},ticks:{font:{size:10,family:"Outfit, sans-serif"}}},y:{grid:{color:"#F3F4F6"},ticks:{font:{size:10,family:"Outfit, sans-serif"},callback:l=>`${l} kW`}}}}})}function cs(e,s){var i;const t=(i=document.getElementById("load-chart"))==null?void 0:i.getContext("2d");if(!t)return;window._loadChart&&window._loadChart.destroy();const n=e.map(a=>a>=s?"#F5A623":"#93C5FD");window._loadChart=new Chart(t,{type:"bar",data:{labels:os,datasets:[{type:"bar",data:e,backgroundColor:n,borderRadius:8,borderSkipped:!1,barPercentage:.55,categoryPercentage:.8,order:2},{type:"line",data:new Array(12).fill(s),borderColor:"#6B7280",borderWidth:1.5,borderDash:[4,3],pointRadius:0,fill:!1,order:1,label:"Monthly average"}]},options:{responsive:!0,plugins:{legend:{display:!1},tooltip:{callbacks:{label:a=>a.datasetIndex===0?`${a.raw} kWh`:`Avg: ${s} kWh`}}},scales:{x:{grid:{display:!1},ticks:{font:{size:10,family:"Outfit, sans-serif"}}},y:{grid:{color:"#F3F4F6"},ticks:{font:{size:10,family:"Outfit, sans-serif"},callback:a=>`${a}`},title:{display:!0,text:"kWh / month",font:{size:10,family:"Outfit, sans-serif"},color:"#9CA3AF"}}}}})}const ds=e=>String(e).replace(/&/g,"&amp;").replace(/"/g,"&quot;");function ps(e){return{Cooling:"❄️",Lighting:"💡",Kitchen:"🍳",Entertainment:"📺","ICT / Office":"💻",Laundry:"🫧",Water:"💧",Security:"🔒"}[e]||"🔌"}function Ge(e){return e>=1e3?`${(e/1e3).toFixed(1)} MWh/yr`:`${e.toLocaleString()} kWh/yr`}function us(){const{results:e,budget:s}=R()||{};if(!e)return;const t=p=>document.getElementById(p),n=p=>"₦"+Number(p).toLocaleString("en-NG"),i=e.savings.total_system_cost,a=s>=i,l=Math.min(100,Math.round(s/i*100));t("fq-solar-kwp")&&(t("fq-solar-kwp").textContent=`${e.solar.panel_kwp.toFixed(1)} kWp`),t("fq-solar-count")&&(t("fq-solar-count").textContent=`${e.solar.panel_count} panels`),t("fq-inverter-kva")&&(t("fq-inverter-kva").textContent=`${e.solar.inverter_kva.toFixed(1)} kVA`),t("fq-battery-kwh")&&(t("fq-battery-kwh").textContent=`${e.battery.battery_kwh.toFixed(1)} kWh`),t("fq-system-cost")&&(t("fq-system-cost").textContent=n(i));const r=t("fq-budget-fill");r&&(r.style.width=`${l}%`);const c=t("fq-budget-delta");c&&(c.style.color=a?"var(--color-success)":"var(--color-error)",c.textContent=a?`+${n(s-i)} surplus`:`${n(i-s)} gap`);const v=t("fq-boq-body");if(v){const p=e.solar,g=e.battery,f=[{product:"Jinko Solar 585W Mono PERC Half-Cell",sku:"JK-585M-HC",category:"Solar Panel",qty:p.panel_count},{product:`DEYE ${p.inverter_kva}kW Hybrid Inverter`,sku:`DEYE-HYB-${p.inverter_kva}KW`,category:"Inverter",qty:1},{product:"48V LiFePO4 Battery 5kWh",sku:"BAT-LFP-48V-5K",category:"Battery",qty:g.battery_units_48v},{product:"Roof Mounting Kit (Tile / Metal)",sku:"MNT-ROOF-TILE",category:"Mounting",qty:Math.ceil(p.panel_count/4)},{product:"4mm² DC Solar Cable (Red + Black)",sku:"CBL-DC-4MM-PAIR",category:"Cabling",qty:`${Math.ceil(p.panel_kwp*10)}m`}];v.innerHTML=f.map(u=>`
      <tr>
        <td><span class="bom-product">${u.product}</span><span class="bom-sku">${u.sku}</span></td>
        <td>${u.category}</td>
        <td style="text-align:right">${u.qty}</td>
      </tr>`).join("")}}function wt(){const e=document.getElementById("pv-confidence-card"),s=document.getElementById("pv-profile-card");if(!e||!s)return;e.style.alignSelf="start";const t=e.getBoundingClientRect().height;e.style.alignSelf="",s.style.maxHeight=t+"px"}function vs(e){return e>=85?"High":e>=55?"Medium":"Low"}function ms(e,s,t){const n=i=>`<div style="padding:12px 14px;background:var(--color-primary-bg);border:1.5px solid var(--color-primary-light);border-radius:var(--radius-md);font-size:12px;line-height:1.6;color:var(--color-text-secondary)">${i}</div>`;return e==="no_appliances"?n(`<strong style="color:var(--color-text)">Boost your confidence score.</strong> Add your appliances and usage schedule to get a <strong>High</strong> confidence result.<div style="margin-top:10px"><button class="btn btn--primary btn--sm" onclick="window._navigate('addAppliances')">Add Appliances →</button></div>`):e==="no_spending"?n('<strong style="color:var(--color-text)">Single-source estimate.</strong> Your sizing is based on your appliance list only. We have no energy spend data to cross-check against.'):e==="variance"&&s==="Low"?n(`<strong style="color:var(--color-text)">Your bills and appliance list don't match up.</strong> ${t==="appliances_higher"?"Your appliance list suggests a much higher consumption than your energy spend implies. Please review your appliance list and make sure it reflects what you actually run.":"Your energy spend implies a much higher load than your appliance list accounts for. Some appliances or heavy loads may be missing from your list."}<div style="margin-top:10px"><button class="btn btn--primary btn--sm" onclick="window._navigate('addAppliances')">Update Appliance List</button></div>`):e==="variance"?n(`<strong style="color:var(--color-text)">Nearly there.</strong> ${t==="appliances_higher"?"Your appliance list suggests slightly more consumption than your energy spend.":"Your energy spend suggests slightly more consumption than your appliance list."} Your sizing is a reasonable estimate. Adjusting your appliance list can bring it closer.<div style="margin-top:10px"><button class="btn btn--primary btn--sm" onclick="window._navigate('addAppliances')">Update Appliance List</button></div>`):""}function jt(e,s){const{results:t,appliances:n}=R();if(!t){s("step1");return}const i=z("appliances")||[],a=z("tariff_bands")||[],l=z("fuel_prices")||[],r=z("generator_efficiency")||[],c=["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"],v=n&&n.length>0,{powerSource:p}=R(),g=p==="grid_only"?"Grid Reliance (Grid)":p==="generator_only"?"Grid Reliance (Gen)":"Grid Reliance (Grid + Gen)",{solarAppliances:f}=R(),u=f?new Set(f):new Set(n.map(_=>_.name));function o(){var $;const _=R(),b=v?n.filter(F=>u.has(F.name)):n,m=new Set(b.map(F=>F.name)),k={..._,appliances:b,customSchedule:_.customSchedule?_.customSchedule.filter(F=>m.has(F.name)):null},S=Wt(k,i,a,l,r),C=It(S,_.location),L=Pt(S,_.goal,_.backupHours),h=qt({hourlyProfile:S.hourlyProfile,pvKWp:C.panel_kwp,batteryKWh:L.battery_kwh,dailyYield:(($=_.location)==null?void 0:$.daily_yield_kwh_per_kwp)||4.5,energyMix:_.powerSource});return{load:S,solar:C,battery:L,dispatch:h}}const{load:d,solar:y,battery:E,dispatch:A}=o(),w=_=>`<span class="confidence-tooltip-wrap" style="display:inline-flex;vertical-align:middle;margin-left:4px"><button class="confidence-tooltip-btn" type="button">?</button><span class="confidence-tooltip-box">${_}</span></span>`;e.innerHTML=`
    <div style="padding:40px 40px 60px">
        ...
        ${v?`
          ...
          <div class="solar-three-col">
            <div class="card">
              ...
              <div class="value value--amber" id="spec-annual-gen" style="font-size:18px;font-weight:700;margin-bottom:12px">${Ge(y.annual_gen_kwh)}</div>
            ...
            <div class="card" id="pv-confidence-card">
              ...
              <div class="confidence-gauge">
                ...
                <div id="spec-confidence-label" style="font-size:18px;font-weight:700;margin-top:-20px">${d.confidenceLabel||vs(d.confidenceScore)}</div>
            ...
            <div class="card" id="pv-profile-card" style="overflow:hidden;display:flex;flex-direction:column;min-height:0">
              ...
              <div class="interactive-profile" style="padding:0;overflow-y:auto;flex:1;min-height:0">
                ${n.map(_=>`
                  <div class="profile-appliance-row" data-name="${ds(_.name)}" style="cursor:pointer;user-select:none">
                    <div class="checkbox ${u.has(_.name)?"checked":""}"></div>
                    <div class="profile-appliance-row__img-placeholder">${ps(_.category||"")}</div>
                    <span>${_.name}</span>
  ...
  `,window._navigate=s,document.querySelectorAll(".confidence-tooltip-wrap").forEach(_=>{var b;(b=_.querySelector(".confidence-tooltip-btn"))==null||b.addEventListener("click",m=>{m.stopPropagation();const k=_.classList.contains("is-open");document.querySelectorAll(".confidence-tooltip-wrap.is-open").forEach(S=>S.classList.remove("is-open")),k||_.classList.add("is-open")})}),document.addEventListener("click",()=>{document.querySelectorAll(".confidence-tooltip-wrap.is-open").forEach(_=>_.classList.remove("is-open"))}),St(y,c),v&&(gs(d.confidenceScore),document.getElementById("spec-confidence-prompt").innerHTML=ms(d.confidenceReason,d.confidenceLabel,d.confidenceDirection),requestAnimationFrame(()=>{Et("dispatch-canvas",A),wt()})),v&&document.querySelectorAll(".profile-appliance-row[data-name]").forEach(_=>{_.addEventListener("click",()=>{const b=_.dataset.name,m=_.querySelector(".checkbox");u.has(b)?(u.delete(b),m.classList.remove("checked")):(u.add(b),m.classList.add("checked"));const k=u.size===n.length;D({solarAppliances:k?null:[...u]});const S=document.getElementById("solar-selection-indicator");S&&(S.textContent=`(${u.size}/${n.length} selected)`),M()})});function M(){const{solar:_,battery:b,dispatch:m}=o(),k=S=>document.getElementById(S);k("spec-solar-kwp").textContent=`${_.panel_kwp} kWp`,k("spec-solar-count").textContent=`Capacity · ${_.panel_count} panels`,k("spec-inverter-kva").textContent=`${_.inverter_kva} kVA`,k("spec-battery-kwh").textContent=`${b.battery_kwh} kWh`,k("spec-battery-units").textContent=`Storage · ${b.battery_units_48v} units`,k("spec-area-m2").textContent=`${_.installation_m2} m²`,k("spec-annual-gen").textContent=Ge(_.annual_gen_kwh),k("spec-storage-cap")&&(k("spec-storage-cap").textContent=`${b.storage_capacity} kWh`),k("spec-storage-out")&&(k("spec-storage-out").textContent=`${b.storage_output.toFixed(2)} kW`),k("spec-backup-ess")&&(k("spec-backup-ess").textContent=`${b.backup_hours_essentials}hrs`),k("spec-backup-app")&&(k("spec-backup-app").textContent=`${b.backup_hours_appliances}hrs`),k("spec-backup-home")&&(k("spec-backup-home").textContent=`${b.backup_hours_whole_home}hrs`),k("dispatch-stat-reliance")&&(k("dispatch-stat-reliance").innerHTML=`${g} <strong>${Math.round(m.gridReliance_before*100)}% → ${Math.round(m.gridReliance_after*100)}%</strong>`),k("dispatch-stat-grid")&&(k("dispatch-stat-grid").innerHTML=`Avg daily grid use <strong>${m.totalDemand.toFixed(1)} → ${m.avgDailyGridKWh} kWh</strong>`),k("dispatch-stat-surplus")&&(k("dispatch-stat-surplus").innerHTML=`Avg daily surplus <strong>${m.dailySurplusKWh} kWh</strong>`),St(_,c),v&&(Et("dispatch-canvas",m),requestAnimationFrame(wt)),$e(),us()}}function St(e,s){var l;const t=document.getElementById("gen-chart");if(!t)return;(l=Chart.getChart(t))==null||l.destroy();const n=t.getContext("2d"),i=e.monthly_gen.map(r=>r.kwh),a=Math.max(...i);new Chart(n,{type:"bar",data:{labels:s,datasets:[{data:i,backgroundColor:i.map(r=>r/a>.9?"#EF4444":r/a>.75?"#F5A623":"#FCD34D"),borderRadius:8,barPercentage:.5,categoryPercentage:.8}]},options:{responsive:!0,plugins:{legend:{display:!1},tooltip:{callbacks:{label:r=>`${r.raw} kWh`}}},scales:{x:{grid:{display:!1},ticks:{font:{size:9,family:"Outfit, sans-serif"}}},y:{grid:{color:"#F3F4F6"},ticks:{font:{size:9,family:"Outfit, sans-serif"},callback:r=>`${r}`}}}}})}function gs(e){var i;const s=document.getElementById("gauge-chart");if(!s)return;(i=Chart.getChart(s))==null||i.destroy();const t=s.getContext("2d"),n=e>=85?"#10B981":e>=55?"#F59E0B":"#EF4444";new Chart(t,{type:"doughnut",data:{datasets:[{data:[e,100-e],backgroundColor:[n,"#E5E7EB"],borderWidth:0,circumference:180,rotation:270}]},options:{responsive:!1,cutout:"75%",plugins:{legend:{display:!1},tooltip:{enabled:!1}}}})}function Et(e,s){let t=document.getElementById(e);if(!t||!s)return;const n=t.cloneNode(!1);t.parentNode.replaceChild(n,t),t=n;const i=t.parentElement,a=window.devicePixelRatio||1,l=i.clientWidth||Math.min(window.innerWidth-32,600),r=220;t.width=l*a,t.height=r*a,t.style.width=l+"px",t.style.height=r+"px";const c=t.getContext("2d");c.scale(a,a);const v=46,p=12,g=10,f=28,u=l-v-p,o=r-g-f,d=s.hours,E=Math.max(...d.map(C=>C.demand+C.solar_to_charge),.01)*1.12,A=o/E,w=u/24,M=Math.max(4,w*.6),_=5,b={solar:"#FCBF1E",battery:"#2E86AB",grid:"#E84855",charge:"#A8DADC"};c.font="10px Outfit, sans-serif",c.textAlign="right",[0,.25,.5,.75,1].forEach(C=>{const L=E*C,h=g+o-L*A;c.strokeStyle="#F3F4F6",c.lineWidth=1,c.beginPath(),c.moveTo(v,h),c.lineTo(v+u,h),c.stroke(),c.fillStyle="#9CA3AF",c.fillText(L.toFixed(1),v-4,h+3.5)}),c.save(),c.translate(13,g+o/2),c.rotate(-Math.PI/2),c.textAlign="center",c.fillStyle="#6B7280",c.fillText("kW",0,0),c.restore();function m(C,L,h,$,F){if($<=0)return;const T=Math.min(F,h/2,$/2);c.beginPath(),c.moveTo(C,L+$),c.lineTo(C,L+T),c.quadraticCurveTo(C,L,C+T,L),c.lineTo(C+h-T,L),c.quadraticCurveTo(C+h,L,C+h,L+T),c.lineTo(C+h,L+$),c.closePath(),c.fill()}const k=[];d.forEach(C=>{const L=[{v:C.solar_to_load,c:b.solar},{v:C.battery_to_load,c:b.battery},{v:C.grid_to_load,c:b.grid},{v:C.solar_to_charge,c:b.charge}];let h=-1;for(let x=L.length-1;x>=0;x--)if(L[x].v>=.001){h=x;break}const $=v+C.hour*w+(w-M)/2,F=g+o;let T=F;L.forEach((x,B)=>{if(x.v<.001)return;const W=x.v*A;T-=W,c.fillStyle=x.c,B===h?m($,T,M,W,_):c.fillRect($,T,M,W)}),k.push({bx:$,bW:M,topY:T,bottomY:F,d:C})}),c.strokeStyle="#D1D5DB",c.lineWidth=1,c.beginPath(),c.moveTo(v,g+o),c.lineTo(v+u,g+o),c.stroke(),c.fillStyle="#6B7280",c.textAlign="center",c.font="9px Outfit, sans-serif";for(let C=0;C<24;C+=3){const L=C===0?"12am":C<12?`${C}am`:C===12?"12pm":`${C-12}pm`;c.fillText(L,v+C*w+w/2,r-6)}const S=document.getElementById("dispatch-tooltip");t.addEventListener("mousemove",C=>{const L=t.getBoundingClientRect(),h=C.clientX-L.left,$=C.clientY-L.top,F=k.find(W=>h>=W.bx&&h<=W.bx+W.bW);if(!F||$<g||$>g+o){S&&(S.style.display="none");return}const T=F.d,x=T.hour,B=x===0?"12am":x<12?`${x}am`:x===12?"12pm":`${x-12}pm`;if(S){let W=h+14,I=$-106;W+152>l&&(W=h-166),I<0&&(I=$+14),S.style.left=W+"px",S.style.top=I+"px",S.style.display="block",S.innerHTML=`
        ...
        <div><span style="color:${b.grid}">■</span> ${s.gridLabel}: ${T.grid_to_load.toFixed(2)} kW</div>
      ...
      `}}),t.addEventListener("mouseleave",()=>{S&&(S.style.display="none")})}const ae=e=>"₦"+Number(e).toLocaleString("en-NG");function Nt(e,s){const t=R();if(!t.results){s("step1");return}const{savings:n}=t.results;t.appliances&&t.appliances.length>0;const i=a=>`<span class="confidence-tooltip-wrap" style="display:inline-flex;vertical-align:middle;margin-left:4px"><button class="confidence-tooltip-btn" type="button">?</button><span class="confidence-tooltip-box">${a}</span></span>`;e.innerHTML=`
    <div style="padding:40px 40px 60px">
  ...
  `,window._navigate=s,document.querySelectorAll(".confidence-tooltip-wrap").forEach(a=>{var l;(l=a.querySelector(".confidence-tooltip-btn"))==null||l.addEventListener("click",r=>{r.stopPropagation();const c=a.classList.contains("is-open");document.querySelectorAll(".confidence-tooltip-wrap.is-open").forEach(v=>v.classList.remove("is-open")),c||a.classList.add("is-open")})}),document.addEventListener("click",()=>{document.querySelectorAll(".confidence-tooltip-wrap.is-open").forEach(a=>a.classList.remove("is-open"))}),ys(n),hs(n)}function ys(e){const s=document.getElementById("cashflow-chart");if(!s)return;const t=window.devicePixelRatio||1,n=s.offsetWidth||Math.min(window.innerWidth-32,500),i=280;s.width=n*t,s.height=i*t;const a=s.getContext("2d");a.scale(t,t);const l=e.cashflow,r=76,c=88,v=28,p=38,g=n-r-c,f=i-v-p,u=l.map(x=>x.cumulative),o=Math.min(...u),d=Math.max(...u),y=(d-o)*.1||Math.abs(o)*.1||1e5,E=o-y,A=d+y,w=A-E||1,M=x=>r+x/25*g,_=x=>v+(1-(x-E)/w)*f,b=_(0),m=l.map(x=>({x:M(x.year),y:_(x.cumulative),year:x.year,cumulative:x.cumulative})),k=m[m.length-1],S=x=>{const B=Math.abs(x),W=x<0?"-₦":"₦";return B>=1e6?`${W}${(B/1e6).toFixed(1)}M`:B>=1e3?`${W}${Math.round(B/1e3)}k`:`${W}${Math.round(B)}`},C=5,L=Array.from({length:C+1},(x,B)=>E+(A-E)*(B/C));a.save(),a.setLineDash([3,4]),a.strokeStyle="#F3F4F6",a.lineWidth=1,L.forEach(x=>{const B=_(x);B<v||B>v+f||(a.beginPath(),a.moveTo(r,B),a.lineTo(r+g,B),a.stroke())}),a.restore(),a.fillStyle="#9CA3AF",a.textAlign="right",a.font="10px Outfit, sans-serif",L.forEach(x=>{const B=_(x);B<v-4||B>v+f+4||a.fillText(S(x),r-7,B+3.5)}),a.strokeStyle="#E5E7EB",a.lineWidth=1,a.setLineDash([]),a.beginPath(),a.moveTo(r,v),a.lineTo(r,v+f),a.stroke();const h=Math.min(Math.max(b,v),v+f);h<v+f&&(a.save(),a.beginPath(),a.rect(r,h,g,v+f-h),a.clip(),a.beginPath(),a.moveTo(m[0].x,m[0].y),m.forEach(x=>a.lineTo(x.x,x.y)),a.lineTo(k.x,h),a.lineTo(m[0].x,h),a.closePath(),a.fillStyle="rgba(232,72,85,0.10)",a.fill(),a.restore()),h>v&&(a.save(),a.beginPath(),a.rect(r,v,g,h-v),a.clip(),a.beginPath(),a.moveTo(m[0].x,m[0].y),m.forEach(x=>a.lineTo(x.x,x.y)),a.lineTo(k.x,h),a.lineTo(m[0].x,h),a.closePath(),a.fillStyle="rgba(34,197,94,0.10)",a.fill(),a.restore()),b>=v&&b<=v+f&&(a.save(),a.setLineDash([6,4]),a.strokeStyle="#9CA3AF",a.lineWidth=1.5,a.beginPath(),a.moveTo(r,b),a.lineTo(r+g,b),a.stroke(),a.restore(),a.fillStyle="#9CA3AF",a.font="bold 10px Outfit, sans-serif",a.textAlign="left",a.fillText("Investment Line",r+g+5,b+4)),a.strokeStyle="#E5E7EB",a.lineWidth=1,a.setLineDash([]),a.beginPath(),a.moveTo(r,v+f),a.lineTo(r+g,v+f),a.stroke(),a.beginPath(),a.moveTo(m[0].x,m[0].y),m.forEach(x=>a.lineTo(x.x,x.y)),a.strokeStyle="#FCBF1E",a.lineWidth=2.5,a.lineJoin="round",a.setLineDash([]),a.stroke(),m.forEach(x=>{a.beginPath(),a.arc(x.x,x.y,2.5,0,Math.PI*2),a.fillStyle="#FCBF1E",a.fill()});const $=e.payback_exact,F=$!=null&&$<99&&$>=0;if(F){const x=M($),B=_(0);a.beginPath(),a.arc(x,B,7,0,Math.PI*2),a.fillStyle="#22C55E",a.fill(),a.strokeStyle="#fff",a.lineWidth=2,a.stroke()}if(F){const x=M($),B=_(0);a.fillStyle="#16A34A",a.font="bold 10px Outfit, sans-serif";const W=x+84>r+g;a.textAlign=W?"right":"center",a.fillText(`Payback: Year ${$}`,W?x-10:x,Math.max(v+14,B-14))}a.fillStyle="#6B7280",a.font="10px Outfit, sans-serif",a.textAlign="center",a.setLineDash([]),[0,3,5,10,15,20,25].forEach(x=>{const B=M(x);a.fillText(`Yr ${x}`,B,v+f+22),a.strokeStyle="#D1D5DB",a.lineWidth=1,a.beginPath(),a.moveTo(B,v+f),a.lineTo(B,v+f+5),a.stroke()}),a.fillStyle="#9CA3AF",a.font="10px Outfit, sans-serif",a.textAlign="center",a.fillText("Year",r+g/2,v+f+36);const T=document.getElementById("cashflow-tooltip");T&&(s.addEventListener("mousemove",x=>{const B=s.getBoundingClientRect(),W=x.clientX-B.left;let I=null,j=1/0;if(m.forEach(H=>{const J=Math.abs(H.x-W);J<j&&(j=J,I=H)}),I&&j<22){const H=I.cumulative>=0?"+":"";T.style.left=I.x+10+"px",T.style.top=Math.max(4,I.y-48)+"px",T.style.display="block";const J=I.year===10?`<div style="color:#FCA5A5;margin-top:3px">Battery replacement: -${ae(e.battery_replacement_cost)}</div>`:"";T.innerHTML=`<div>Year ${I.year}: ${H}${ae(I.cumulative)}</div>${J}`}else T.style.display="none"}),s.addEventListener("mouseleave",()=>{T.style.display="none"}))}function hs(e){var t;const s=(t=document.getElementById("compare-chart"))==null?void 0:t.getContext("2d");s&&new Chart(s,{type:"bar",data:{labels:[e.current_label||"Grid+Gen",e.solar_label||"Solar"],datasets:[{data:[e.current_monthly_cost,e.post_solar_monthly_cost],backgroundColor:["#E74C3C","#1B4F72"],borderRadius:6,barThickness:60}]},options:{responsive:!0,plugins:{legend:{display:!1},tooltip:{callbacks:{label:n=>`₦${Number(n.raw).toLocaleString()}`}}},scales:{x:{grid:{display:!1},title:{display:!0,text:"Monthly Cost Scenario",font:{size:11,family:"Outfit, sans-serif"}},ticks:{font:{size:10,family:"Outfit, sans-serif"}}},y:{grid:{color:"#F3F4F6"},suggestedMax:Math.max(e.current_monthly_cost,e.post_solar_monthly_cost)*1.4,ticks:{font:{size:10,family:"Outfit, sans-serif"},callback:n=>`₦${(n/1e3).toFixed(0)}k`}}}},plugins:[{id:"barValueLabels",afterDatasetsDraw(n){const{ctx:i,data:a}=n,l=n.getDatasetMeta(0);i.save(),i.font="bold 11px Outfit, sans-serif",i.textAlign="center",i.textBaseline="bottom",l.data.forEach((r,c)=>{const v=a.datasets[0].data[c];i.fillStyle="#374151",i.fillText("₦"+Number(v).toLocaleString("en-NG"),r.x,r.y-4)}),i.restore()}}]})}function zt(e,s){var t,n;e.innerHTML=`
    <div class="cta-outer">
  ...
  `}function ks(){var e;(e=document.getElementById("modal-close-btn"))==null||e.addEventListener("click",Le)}const At=["Work From Home","Office Worker","Night Shift","Stay-at-Home","Student","Weekend"],xs={Cooling:"#3B82F6",Lighting:"#FCBF1E",Kitchen:"#F59E0B",Entertainment:"#8B5CF6","ICT / Office":"#06B6D4",Laundry:"#10B981",Water:"#0EA5E9",Security:"#EF4444"};function _s(e,s){switch(s){case"Cooling":return/fan/i.test(e)?"Ceiling Fan":/\b1hp\b/i.test(e)?"AC – Bedroom":"AC – Living Room";case"Lighting":return"Lighting";case"Entertainment":return"Television";case"ICT / Office":return"Laptop / PC";case"Laundry":return/washing/i.test(e)?"Washing Machine":null;case"Kitchen":return/refrig|freezer/i.test(e)?"Refrigerator":/kettle/i.test(e)?"Electric Kettle":null;case"Water":return/pump/i.test(e)?"Water Pump":null;case"Security":return"Always On";default:return null}}function ws(e){const s=[];let t=!1,n=0;for(let i=0;i<24;i++)e[i]>=.5&&!t?(t=!0,n=i):e[i]<.5&&t&&(s.push({start:n,end:i}),t=!1);return t&&s.push({start:n,end:24}),s.length?s:[{start:8,end:10}]}function $t(e,s,t){if(!t)return[{start:8,end:10}];const n=_s(e,s),i=n&&t.schedule[n];return i?ws(i):[{start:8,end:10}]}function be(e){return Math.round(e*2)/2}function ue(e){const s=Math.floor(e)%24;return s===0?"12am":s<12?`${s}am`:s===12?"12pm":`${s-12}pm`}const P={active:!1,type:null,rowIdx:null,segIdx:null,startX:null,origStart:null,origEnd:null,trackWidth:null};function Ss(e,s){const t=document.getElementById(e);if(!t)return()=>{};const n=z("usage_patterns")||[];let i=R().usagePattern||At[0];const a=R().customSchedule||[],l=Object.fromEntries(a.map(o=>[o.name,o])),r=n.find(o=>o.pattern===i);let c=s.map(o=>l[o.name]?{...l[o.name],segments:l[o.name].segments.map(d=>({...d}))}:{name:o.name,category:o.category||"",segments:$t(o.name,o.category||"",r)});function v(){D({customSchedule:c.map(o=>({...o,segments:o.segments.map(d=>({...d}))}))})}function p(){const o=t.querySelector(".gantt-body");o&&(o.innerHTML=c.map((d,y)=>{const E=xs[d.category]||"#9CA3AF",A=d.segments.map((w,M)=>{const _=(w.start/24*100).toFixed(3),b=Math.max(.5,(w.end-w.start)/24*100).toFixed(3);return`
          <div class="gantt-bar" data-ri="${y}" data-si="${M}"
        ...
        <div class="gantt-row">
          ...
          <div class="gantt-row__track" data-ri="${y}" style="background:${E}22">${A}</div>
        </div>`}).join(""),g())}function g(){t.querySelectorAll(".gantt-bar__del").forEach(o=>{o.addEventListener("click",d=>{d.stopPropagation();const y=o.closest(".gantt-bar");c[+y.dataset.ri].segments.splice(+y.dataset.si,1),v(),p()})}),t.querySelectorAll(".gantt-bar").forEach(o=>{o.addEventListener("contextmenu",d=>{d.preventDefault(),c[+o.dataset.ri].segments.splice(+o.dataset.si,1),v(),p()}),o.addEventListener("mousedown",d=>{if(d.target.classList.contains("gantt-bar__handle")||d.target.classList.contains("gantt-bar__del"))return;d.preventDefault();const y=+o.dataset.ri,E=+o.dataset.si;P.active=!0,P.type="move",P.rowIdx=y,P.segIdx=E,P.startX=d.clientX,P.origStart=c[y].segments[E].start,P.origEnd=c[y].segments[E].end,P.trackWidth=o.closest(".gantt-row__track").getBoundingClientRect().width})}),t.querySelectorAll(".gantt-bar__handle").forEach(o=>{o.addEventListener("mousedown",d=>{d.preventDefault(),d.stopPropagation();const y=o.closest(".gantt-bar"),E=+y.dataset.ri,A=+y.dataset.si;P.active=!0,P.type=o.dataset.type,P.rowIdx=E,P.segIdx=A,P.startX=d.clientX,P.origStart=c[E].segments[A].start,P.origEnd=c[E].segments[A].end,P.trackWidth=y.closest(".gantt-row__track").getBoundingClientRect().width})}),t.querySelectorAll(".gantt-row__track").forEach(o=>{o.addEventListener("dblclick",d=>{if(d.target!==o)return;const y=+o.dataset.ri,E=o.getBoundingClientRect(),A=be((d.clientX-E.left)/E.width*24),w=Math.max(0,Math.min(22,A));c[y].segments.push({start:w,end:Math.min(24,w+2)}),v(),p()})})}function f(o){if(!P.active)return;const d=(o.clientX-P.startX)/P.trackWidth*24,{type:y,rowIdx:E,segIdx:A}=P,w=c[E].segments[A],M=P.origEnd-P.origStart;if(y==="move"){const b=Math.max(0,Math.min(24-M,be(P.origStart+d)));w.start=b,w.end=b+M}else y==="resize-left"?w.start=Math.max(0,Math.min(P.origEnd-.5,be(P.origStart+d))):w.end=Math.min(24,Math.max(P.origStart+.5,be(P.origEnd+d)));const _=t.querySelector(`.gantt-bar[data-ri="${E}"][data-si="${A}"]`);if(_){_.style.left=`${(w.start/24*100).toFixed(3)}%`,_.style.width=`${Math.max(.5,(w.end-w.start)/24*100).toFixed(3)}%`;const b=_.querySelector(".gantt-bar__label");b&&(b.textContent=`${ue(w.start)}–${ue(w.end)}`)}}function u(){P.active&&(P.active=!1,v())}return document.addEventListener("mousemove",f),document.addEventListener("mouseup",u),t.innerHTML=`
      ...
      <div class="gantt-header">
        ...
        <div class="gantt-pattern-row">
          ...
          <select class="gantt-select" id="gantt-pattern-sel">
            ${At.map(o=>`<option value="${o}" ${i===o?"selected":""}>${o}</option>`).join("")}
          </select>
        </div>
      </div>
    ...
    </div>`,document.getElementById("gantt-pattern-sel").addEventListener("change",o=>{i=o.target.value,D({usagePattern:i});const d=n.find(y=>y.pattern===i);c=c.map(y=>({...y,segments:$t(y.name,y.category,d)})),v(),p()}),p(),v(),function(){document.removeEventListener("mousemove",f),document.removeEventListener("mouseup",u)}}const Ne=e=>String(e).replace(/&/g,"&amp;").replace(/"/g,"&quot;");function Kt(e){return{Cooling:"❄️",Lighting:"💡",Kitchen:"🍳",Entertainment:"📺","ICT / Office":"💻",Laundry:"🫧",Water:"💧",Security:"🔒"}[e]||"🔌"}function Es(e,s){const t={};s.forEach(i=>{t[i.name]=i});const n=e.map(i=>{const a=t[i.name],l=a?a.qty:0;return`
    ...
    `}).join("");return bs({title:"Choose specific appliances",subtitle:"You can select multiple appliances",body:n,footer:'<button class="btn btn--primary btn--full" id="add-appliances-confirm">Add Appliances</button>'})}const As=`
...
`,$s={bungalow:9,duplex:14,terrace:20},Lt={"9W":2,"15W":1};function Ls(e,s,t){const n=(t[e]||[]).filter(a=>!a.name.startsWith("LED Bulb")),i=s&&s>0?s:$s[e]||9;return[...n,{name:"LED Bulb (9W)",qty:i*Lt["9W"]},{name:"LED Bulb (15W)",qty:i*Lt["15W"]}]}function Cs(e,s){const t=z("appliances")||[],n=z("house_type_appliances")||{};let i=null;const a=[{id:"bungalow",emoji:'<img src="/icons/bungalow.png" width="72" height="72" style="object-fit:contain">',name:"Bungalow"},{id:"duplex",emoji:'<img src="/icons/duplex_home_type.png" width="72" height="72" style="object-fit:contain">',name:"Duplex"},{id:"terrace",emoji:'<img src="/icons/terrace_house_type.png" width="72" height="72" style="object-fit:contain">',name:"Terrace House"}];function l(){i&&(i(),i=null);const o=R();e.innerHTML=`
  ...
  `}function gi(e){var i;const s=(i=R().location)==null?void 0:i.state,t=Ye(s);return`
    <div class="mk-map">
      ...
      <!-- District labels -->
      ${t.labels.map((a,l)=>({label:a,...ci[l]})).map(a=>`<div class="mk-district" style="left:${a.x}%;top:${a.y}%">${a.label}</div>`).join("")}
  ...
  `}function yi(){const e=q.shortlist;if(e.length===0)return"";const s=Me(Ce),t=e.map(i=>s.find(a=>a.id===i)).filter(Boolean),n=e.length<2?"Add one more to compare side by side":e.length>=4?"Maximum 4 quotes reached":"Ready to compare side by side";return`
        ...
        <div class="sf-body">
          <div class="sf-quote-panel">
            <div style="flex:1;min-width:0">
              <div class="sf-quote-label">Their quote for your ${((u=(f=(g=R().results)==null?void 0:g.solar)==null?void 0:f.panel_kwp)==null?void 0:u.toFixed(2))??"N/A"} kWp system</div>
              ...
              <div class="sf-review-card">
                <div class="sf-review-top">
                  <div style="display:flex;align-items:center;gap:10px">
                    <div class="sf-review-avatar" style="background:${i[d%3]}">${o.name[0]}</div>
                    <div>
  ...
  `,e.querySelector("#sf-back").addEventListener("click",()=>{q.view="market",ge(),requestAnimationFrame(Ut)}),e.querySelector("#sf-toggle").addEventListener("click",()=>Ee(t.id))}function fi(e){return ce[e]||ce["Abuja (FCT)"]}const Jt='<svg width="14" height="14" viewBox="0 0 14 14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="square" stroke-linejoin="miter" style="display:inline-block;vertical-align:middle;margin-right:5px"><polyline points="9,2 4,7 9,12"/></svg>',bi=[{id:"baobab",name:"Baobab MFB",init:"BA",rate:2.8,approval:"3–5 days",maxTenure:36,color:"#1F2937"},{id:"lapo",name:"LAPO MFB",init:"LA",rate:3,approval:"3–5 days",maxTenure:36,color:"#1E5BB8"},{id:"ab",name:"AB Microfinance",init:"AB",rate:3.1,approval:"2–4 days",maxTenure:30,color:"#3D6B7A"},{id:"accion",name:"Accion MFB",init:"AC",rate:3.3,approval:"3–5 days",maxTenure:36,color:"#C0392B"},{id:"renmoney",name:"Renmoney",init:"RE",rate:3.5,approval:"24–48 hrs",maxTenure:24,color:"#6A2FB8"},{id:"fairmoney",name:"FairMoney MFB",init:"FA",rate:3.8,approval:"Same day",maxTenure:24,color:"#0E8F6E"}];function ki(e,s,t){const n=e/100;return t*n/(1-Math.pow(1+n,-s))}let V=null;function Mt(e){V=e,O.view="config"}let O={down:30,tenure:36,picked:null,view:"config",fundMode:"loan"},N=null,Ve=null;function ve(){return V!=null&&V.price?V.price:462e4}function xi(){var e,s,t;return((t=(s=(e=R().results)==null?void 0:e.solar)==null?void 0:s.panel_kwp)==null?void 0:t.toFixed(2))??"N/A"}function _i(e,s){N=e,Ve=s,Z()}function Z(){var A,w,M,_;if(O.view==="done"){Ei();return}const e=ve(),s=O.fundMode!=="self",t=e*(1-O.down/100),n=e*O.down/100,i=bi.map(b=>{const m=Math.min(O.tenure,b.maxTenure),k=ki(b.rate,m,t),S=k*m+n;return{...b,n:m,monthly:k,total:S}}).sort((b,m)=>b.monthly-m.monthly),a=["fairmoney","renmoney"],l=[...i.filter(b=>a.includes(b.id)),...i.filter(b=>!a.includes(b.id))],r=i[0],c=(O.down-10)/40*100,v=`linear-gradient(to right, #FCBF1E 0%, #FCBF1E ${c}%, #E5E7EB ${c}%, #E5E7EB 100%)`,p=V,g=(p==null?void 0:p.init)??"SO",f=(p==null?void 0:p.name)??"Your installer",o=((A=R().location)==null?void 0:A.state)??"Abuja (FCT)",d=o.replace(/\s*\(FCT\)/i,"").trim(),y=p?fi(o).installerAreas[p.id]||p.district:null,E=[p!=null&&p.warranty?p.warranty+" warranty":null,y?y+", "+d:null].filter(Boolean).join(" · ");if(N.innerHTML=`
    <div class="mk-nav-bar">
          ...
          <div class="fc-card">
            ...
            <div class="fc-system-row">
              <div class="fc-system-info">
                ...
                <div>
                  <div class="fc-system-name">${f} · ${xi()} kWp system</div>
  ...
  `,(w=N.querySelector("#fc-mode-switch"))==null||w.addEventListener("click",()=>{O.fundMode=s?"self":"loan",Z()}),N.querySelectorAll("[data-mode]").forEach(b=>{b.addEventListener("click",()=>{O.fundMode=b.dataset.mode,Z()})}),s){const b=N.querySelector("#fc-range");b.addEventListener("input",()=>{const m=parseInt(b.value);O.down=m;const k=(m-10)/40*100;b.style.background=`linear-gradient(to right, #FCBF1E 0%, #FCBF1E ${k}%, #E5E7EB ${k}%, #E5E7EB 100%)`;const S=N.querySelector("#fc-down-pct");S&&(S.textContent=`${m}%`);const C=ve(),L=N.querySelector("#fc-down-row");L&&(L.innerHTML=`<span>You pay now: <strong>${G(C*m/100)}</strong></span><span>To finance: <strong>${G(C*(1-m/100))}</strong></span>`)}),b.addEventListener("change",()=>Z()),N.querySelectorAll("[data-tenure]").forEach(m=>{m.addEventListener("click",()=>{O.tenure=parseInt(m.dataset.tenure),Z()})}),N.querySelectorAll("[data-bank]").forEach(m=>{m.addEventListener("click",()=>{const k=i.find(S=>S.id===m.dataset.bank);k&&wi(k,n,t)})})}else(M=N.querySelector('[data-bank="self"]'))==null||M.addEventListener("click",()=>{O.picked={id:"self",name:"self-fund",isSelf:!0,monthly:0,total:ve(),approval:"N/A"},O.view="done",Z()});(_=N.querySelector("#fc-back"))==null||_.addEventListener("click",()=>Ve("compare"))}function wi(e,s,t){var l,r,c;const n=ve(),i=V,a=document.createElement("div");a.id="fc-modal-overlay",a.innerHTML=`
    <div class="fc-modal">
      ...
      ${i?`<div class="fc-modal-installer">
        ...
        <span style="font-size:13px;font-weight:600;color:var(--color-text)">${i.name} · ${((c=(r=(l=R().results)==null?void 0:l.solar)==null?void 0:r.panel_kwp)==null?void 0:c.toFixed(2))??"N/A"} kWp system</span>
      </div>`:""}
  ...
  `,N.querySelector("#fc-adjust").addEventListener("click",()=>{O.view="config",Z()}),(l=N.querySelector("#fc-done-back"))==null||l.addEventListener("click",()=>Ve("compare"))}function Ai(e){return ce[e]||ce["Abuja (FCT)"]}const $i='<svg width="14" height="14" viewBox="0 0 14 14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="square" stroke-linejoin="miter" style="display:inline-block;vertical-align:middle;margin-right:5px"><polyline points="9,2 4,7 9,12"/></svg>',Li='<svg width="14" height="14" viewBox="0 0 14 14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="square" stroke-linejoin="miter" style="display:inline-block;vertical-align:middle;margin-left:5px"><polyline points="5,2 10,7 5,12"/></svg>',Ci=[{label:"Total quote",key:"price",render:e=>G(e.price),bestFn:e=>e.reduce((s,t)=>t.price<s.price?t:s),bestTag:"Lowest"},{label:"Value score",key:"score",render:e=>`<strong>${e.score}/100</strong>`,bestFn:e=>e.reduce((s,t)=>t.score>s.score?t:s),bestTag:"Top"},{label:"Warranty",key:"warranty",render:e=>e.warranty,bestFn:e=>e.reduce((s,t)=>t.warrantyScore>s.warrantyScore?t:s),bestTag:"Longest"},{label:"Install time",key:"timeline",render:e=>e.timeline,bestFn:null},{label:"Solar panels",key:"panel",render:e=>e.panel,bestFn:null},{label:"Battery",key:"battery",render:e=>e.battery,bestFn:null},{label:"Inverter",key:"inverter",render:e=>e.inverter,bestFn:null},{label:"After-sales",key:"aftercare",render:e=>e.aftercare,bestFn:null},{label:"Rating",key:"rating",render:e=>`★ ${e.rating} (${e.reviews})`,bestFn:e=>e.reduce((s,t)=>t.rating>s.rating?t:s),bestTag:"Best"},{label:"Financing",key:"financing",render:e=>e.financing,bestFn:null}];let X=null,xe=null;function Mi(e,s){X=e,xe=s,He()}function He(){var g,f;const e=Me(Ce),s=q.shortlist,t=s.length>0?s:[e.sort((u,o)=>o.score-u.score)[0].id],n=t.map(u=>e.find(o=>o.id===u)).filter(Boolean),i=n.reduce((u,o)=>o.score>u.score?o:u,n[0]),a=i==null?void 0:i.id,l=e.filter(u=>!t.includes(u.id)),r=n.length<4&&l.length>0,c=n.length>1?`Comparing ${n.length} quotes for your home`:"Add installers to compare";X.innerHTML=`
    <div class="mk-nav-bar">
      <button class="btn--dark-outline" style="font-size:12px;padding:7px 14px;display:inline-flex;align-items:center;gap:4px" id="cq-back">${$i}Back to installers</button>
    </div>
    <div class="cq-page">
      ...
      <div class="cq-banner">
        ...
        <div class="cq-banner-right">
          <div class="cq-banner-price">${G(i.price)}</div>
          <div class="cq-banner-score">Value score ${i.score}/100</div>
          <button class="cq-pick-btn" id="cq-pick-best">Select &amp; Finance ${Li}</button>
        </div>
      </div>
        ...
        <table class="cq-table">
          <thead>
            <tr>
              ...
              ${n.map(u=>{var d;const o=Ai((d=R().location)==null?void 0:d.state).installerAreas[u.id]||u.district;return`
          ...
          <tbody>
            ${Ci.map((u,o)=>{const d=o%2===0,y=d?"#F2F3F2":"#FFFFFF",E="#FEF0C0",A="#FEF8E0";return`
              <tr>
                <td class="cq-col-label" style="background:${y}">${u.label}</td>
                ${n.map(w=>{var m;const M=u.bestFn&&((m=u.bestFn(n))==null?void 0:m.id)===w.id,_=w.id===a?d?E:A:y;return`<td class="${`cq-td ${w.id===a?"cq-td--best":""}`}" style="background:${_}">
                    <span ${M?'class="cq-td--green"':""}>${u.render(w)}</span>
                    ${M?`<span class="cq-best-tag">${u.bestTag}</span>`:""}
                  </td>`}).join("")}
                ${r?`<td class="cq-td" style="background:${y}"></td>`:""}
              </tr>`}).join("")}
            <tr>
  ...
  `,Nt(document.getElementById("section-costSavings"),K),Dt(document.getElementById("section-loadProfile"),K),jt(document.getElementById("section-solarPVSystem"),K),zt(document.getElementById("section-finalQuote"),K),se=Wi(),e&&requestAnimationFrame(()=>Zt(e))}function Xt(){const e=document.getElementById("results-content");if(e&&e.scrollHeight>e.clientHeight)return e;const s=document.querySelector(".right-panel");return s&&s.scrollHeight>s.clientHeight?s:null}function Zt(e){const s=document.getElementById(`section-${e}`);if(!s)return;const t=Xt();if(t){const n=t.getBoundingClientRect(),i=s.getBoundingClientRect();t.scrollTo({top:t.scrollTop+i.top-n.top,behavior:"smooth"})}else s.scrollIntoView({behavior:"smooth",block:"start"})}function Wi(){const e=Xt(),s=e||window;function t(){const n=e?e.clientHeight:window.innerHeight,a=(e?e.getBoundingClientRect().top:0)+n*.3;let l=Ae[0];for(const r of Ae){const c=document.getElementById(`section-${r}`);c&&c.getBoundingClientRect().top<=a&&(l=r)}l!==U&&(U=l,Qt(l))}return s.addEventListener("scroll",t,{passive:!0}),()=>s.removeEventListener("scroll",t)}function Qt(e){document.querySelectorAll("#results-nav [data-route]").forEach(s=>{s.classList.toggle("active",s.dataset.route===e)})}function ea(){const e=document.getElementById("wizard-layout"),s=document.getElementById("results-layout");if(se&&(se(),se=null),Fi.includes(U)){e.classList.remove("hidden"),s.classList.add("hidden");const t=document.getElementById("wizard-content");t.innerHTML="",{step1:Tt,step2:Rt,step3:Ot}[U](t,K),document.querySelector(".right-panel").scrollTop=0}else if(["addAppliances","market","compare","financing"].includes(U)){e.classList.add("hidden"),s.classList.remove("hidden"),ta(),aa();const t=document.getElementById("results-content");t.innerHTML="",Bi[U](t,K),requestAnimationFrame(()=>{t.scrollTop=0})}}function ta(){var n;const e=document.getElementById("results-nav"),s=document.getElementById("results-actions"),t=U;e.innerHTML=Ti.map(i=>`
